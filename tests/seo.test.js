...
test("[SEO 2026] Linking: crawl depth — все статьи достижимы с главной за ≤2 клика", () => {
  ...
  const scenarioHtml = fs.readFileSync(scenarioPage, "utf8");
  const tshootHtml = fs.readFileSync(troubleshootingPage, "utf8");

  const scenarioLinks = new Set([...scenarioHtml.matchAll(/href="\/scenarios\/([^"#]+)/g)].map(m => m[1]));
  const tshootLinks = new Set([...tshootHtml.matchAll(/href="\/troubleshooting\/([^"#]+)/g)].map(m => m[1]));

  const allScenarioArticles = new Set(articleFiles.filter(a => a.data.category === "scenario").map(a => a.name.replace(".md", "")));
  const allTroubleshootingArticles = new Set(articleFiles.filter(a => a.data.category === "troubleshooting").map(a => a.name.replace(".md", "")));

  allScenarioArticles.forEach(slug => {
    assert.ok(scenarioLinks.has(slug), `/scenarios/${slug} не найден на странице /scenarios/`);
  });
  allTroubleshootingArticles.forEach(slug => {
    assert.ok(tshootLinks.has(slug), `/troubleshooting/${slug} не найден на странице /troubleshooting/`);
  });
...
test("[SEO 2026] Linking: related статьи используют правильные URL-пути", () => {
  ...
  files.forEach(fp => {
    ...
    links.forEach(link => {
      assert.ok(link.includes("/scenarios/") || link.includes("/troubleshooting/"),
...
test("[SEO 2026] Hubs: категорийные страницы не выдают себя за статьи (нет хлебных крошек, нет article:published_time)", () => {
  const hubFiles = [path.join(DIST_DIR, "scenarios/index.html"), path.join(DIST_DIR, "troubleshooting/index.html")];
  hubFiles.forEach(fp => {
    if (!fs.existsSync(fp)) return;
    const content = fs.readFileSync(fp, "utf8");
...
test("[SEO 2026] Header: навигация использует чистые URL вместо хеш-фрагментов", () => {
  ...
  files.forEach(fp => {
    ...
    assert.doesNotMatch(content, /href="\/#troubleshooting"/, `${fp}: header содержит устаревшую ссылку /#troubleshooting`);
  });
});
